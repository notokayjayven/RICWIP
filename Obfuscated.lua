...
pulseexec.Text = "Enter [name] here"
...
function findpotion()
    for i, v in pairs(game.Players:GetChildren()) do
        local check = v.Backpack:findFirstChild("OddPotion")
        if check then
            return check
        end
    end
...
headbox.FocusLost:Connect(function(enterpressed, inputs)
    ...
    if findplr and enterpressed and findplr.headval.Value == false then
        ...
        potion.TransEvent:FireServer(workspace[headbox.Text]["Head"], 1)
        potion.TransEvent:FireServer(workspace[headbox.Text]["Head"].face, 1)
    ...
    elseif not findplr and enterpressed and (headbox.Text == "all") and workspace.allhead.Value == false then
        ...
        local potion = findpotion()
        for _, v in pairs(game.Players:GetChildren()) do
             potion.TransEvent:FireServer(v.Character["Head"], 1)
             potion.TransEvent:FireServer(v.Character["Head"].face, 1)
        end
        headbox.Text = ""
    elseif not findplr and enterpressed and (headbox.Text == "all") and workspace.allhead.Value == true then
        ...
        local potion = findpotion()
        for _, v in pairs(game.Players:GetChildren()) do
             potion.TransEvent:FireServer(v.Character["Head"], 0)
             potion.TransEvent:FireServer(v.Character["Head"].face, 0)
        end
        headbox.Text = ""
    elseif findplr and enterpressed and findplr.headval.Value == true then
        print("working..")
        local potion = findpotion()
        potion.TransEvent:FireServer(workspace[headbox.Text]["Head"], 0)
        potion.TransEvent:FireServer(workspace[headbox.Text]["Head"].face, 0)
...
baldbox.FocusLost:Connect(function(enterpressed, inputs)
    ...
    if findplr and enterpressed and findplr.baldval.Value == false then
        ...
        for _, h in pairs(game.Players[baldbox.Text].Character:GetChildren()) do
            if h.ClassName == "Accessory" then
             potion.TransEvent:FireServer(h.Handle, 1)
             --wait()
            end
        end
    ...
    elseif not findplr and enterpressed and (baldbox.Text == "all") and workspace.allbald.Value == true then
       ...
       for i, v in pairs(game.Players:GetChildren()) do
            for _, h in pairs(v.Character:GetChildren()) do
                 if h.ClassName == "Accessory" then
                     potion.TransEvent:FireServer(h.Handle, 0)
                 end
            end
            wait()
       end
...
pulseexec.FocusLost:Connect(function(enterpressed, inputs)
    local findplr = game.Players:findFirstChild(pulseexec.Text)
    print("-")
    if findplr and enterpressed then
        print("working..")
        local trans = 0
        repeat
            for i, v in pairs(game.Players[pulseexec.Text].Character:GetChildren()) do
                local potion = findpotion()
                if v.ClassName == "Accessory" then
                    for b = 0, 10 do
                        potion.TransEvent:FireServer(v.Handle, trans)
                        trans = trans + .1
                        wait(.1)
                        print(trans)
                    end
        ...
        until game.Players[pulseexec.Text].Character.Humanoid.Health == 0
        pulseexec.Text = ""
    elseif not findplr and enterpressed then
        pulseexec.Text = ""
        pulseexec.Text = "Not a player!"
    end
end)
